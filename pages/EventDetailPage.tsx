  ...
  return (
    <div className="min-h-screen bg-white pt-0">
      ...
      <div className="fixed top-0 left-0 right-0 z-30 p-4 sm:p-4 flex items-center justify-between lg:hidden pointer-events-none safe-area-inset-top">
         <button onClick={() => setViewState(ViewState.FEED)} className="w-11 h-11 sm:w-10 sm:h-10 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center text-[#15383c] shadow-lg pointer-events-auto hover:scale-105 active:scale-[0.92] transition-transform touch-manipulation border border-white/50"><ChevronLeft size={22} className="sm:w-6 sm:h-6" /></button>
         <div className="flex gap-2.5 sm:gap-3 pointer-events-auto">
             <button className="w-11 h-11 sm:w-10 sm:h-10 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center text-[#15383c] shadow-lg hover:scale-105 active:scale-[0.92] transition-transform touch-manipulation border border-white/50"><Share2 size={20} className="sm:w-5 sm:h-5" /></button>
             {isLoggedIn && onToggleFavorite && (<button onClick={(e) => onToggleFavorite(e, event.id)} className="w-11 h-11 sm:w-10 sm:h-10 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center text-[#15383c] shadow-lg hover:scale-105 active:scale-[0.92] transition-transform touch-manipulation border border-white/50"><Heart size={20} className="sm:w-5 sm:h-5" fill={favorites.includes(event.id) ? "#e35e25" : "none"} style={{ color: favorites.includes(event.id) ? "#e35e25" : "currentColor" }} /></button>)}
         </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 md:py-10 lg:py-12 grid lg:grid-cols-3 gap-6 sm:gap-8 md:gap-10 lg:gap-12">
        <div className="lg:col-span-2 space-y-6 sm:space-y-8 md:space-y-10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 p-5 sm:p-6 md:p-7 lg:p-8 bg-gray-50 rounded-2xl sm:rounded-3xl border border-gray-100 hover:border-gray-200 transition-colors">
            <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto min-w-0">
               ...
               <div className="min-w-0 flex-1"><p className="text-[10px] sm:text-xs uppercase tracking-wider text-gray-500 font-bold mb-1">Hosted by</p><h3 className="text-base sm:text-lg md:text-xl font-bold text-[#15383c] cursor-pointer hover:text-[#e35e25] transition-colors truncate" onClick={() => onHostClick(event.hostName)}>{event.hostName}</h3><button onClick={(e) => onReviewsClick(e, event)} className="flex items-center space-x-1 mt-1.5 bg-white hover:bg-orange-50 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-lg transition-colors border border-gray-200 hover:border-orange-100 group/rating shrink-0 w-fit touch-manipulation active:scale-95"><Star size={11} className="sm:w-3 sm:h-3 text-gray-300 group-hover/rating:text-[#e35e25] group-hover/rating:fill-[#e35e25] transition-colors" fill="currentColor" /><span className="text-[10px] sm:text-xs font-bold text-[#15383c]">{event.rating}</span><span className="text-[9px] sm:text-[10px] text-gray-400 group-hover/rating:text-orange-400">({event.reviewCount})</span></button></div>
            </div>
            <button onClick={() => onHostClick(event.hostName)} className="w-full sm:w-auto px-5 sm:px-6 md:px-8 py-2.5 sm:py-3 bg-white border border-gray-200 rounded-full text-sm md:text-base font-bold text-[#15383c] hover:border-[#e35e25] hover:text-[#e35e25] transition-colors shadow-sm whitespace-nowrap touch-manipulation active:scale-95">View Profile</button>
          </div>

          <div className="grid grid-cols-2 gap-4 sm:gap-5">
             <div className="bg-gray-50 p-5 sm:p-6 md:p-7 rounded-2xl border border-gray-100 text-center"><h4 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-[#15383c]">{event.attendees || 0}</h4><p className="text-xs sm:text-sm uppercase tracking-wide text-gray-500 font-bold mt-1.5">Attending</p></div>
             <div className="bg-gray-50 p-5 sm:p-6 md:p-7 rounded-2xl border border-gray-100 text-center"><h4 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-[#15383c]">{event.capacity || 'Unlimited'}</h4><p className="text-xs sm:text-sm uppercase tracking-wide text-gray-500 font-bold mt-1.5">Capacity</p></div>
          </div>

          <div>
            <h2 className="text-lg sm:text-xl md:text-2xl font-heading font-bold text-[#15383c] mb-3 sm:mb-4 md:mb-6 flex items-center gap-2 sm:gap-3">About this event</h2>
          ...
          <div>
             <h2 className="text-lg sm:text-xl md:text-2xl font-heading font-bold text-[#15383c] mb-3 sm:mb-4 md:mb-6 flex items-center gap-2 sm:gap-3"><Sparkles size={18} className="sm:w-5 sm:h-5 text-[#e35e25]" /> What to Expect</h2>
             <div className="bg-[#eef4f5] p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-[1.5rem] md:rounded-[2rem] border border-[#15383c]/5 text-gray-600 font-light leading-relaxed text-sm sm:text-base"><p className="mb-3 sm:mb-4">An immersive experience designed to connect you with like-minded individuals.</p></div>
          </div>

          <div className="border-t border-gray-100 pt-6 sm:pt-8 md:pt-10">
            <h2 className="text-lg sm:text-xl md:text-2xl font-heading font-bold text-[#15383c] mb-4 sm:mb-6">Location</h2>
            ...
            <p className="mt-3 sm:mt-4 md:mt-6 text-[#15383c] font-medium text-sm sm:text-base md:text-lg flex items-center">
              <MapPin size={16} className="sm:w-[18px] sm:h-[18px] mr-1.5 sm:mr-2 md:mr-3 text-[#e35e25] shrink-0" /> 
        ...
        <div className="relative hidden lg:block">
           <div className="sticky top-32 bg-white rounded-[2.5rem] shadow-[0_20px_40px_rgba(0,0,0,0.04)] border border-gray-100 p-6 lg:p-8">
              <div className="flex justify-between items-center mb-6 lg:mb-8"><div><span className="text-3xl lg:text-4xl font-heading font-bold text-[#15383c]">{event.price}</span><p className="text-xs lg:text-sm text-gray-500 font-medium mt-1">per person</p></div></div>
      ...
      <section className="bg-[#15383c] py-10 sm:py-12 md:py-16 lg:py-20 relative overflow-hidden">
         ...
         <div className="max-w-4xl mx-auto px-4 sm:px-6 relative z-10 text-center">
            ...
            <button className="px-8 sm:px-10 py-3.5 sm:py-4 bg-[#e35e25] text-white rounded-full font-bold text-base sm:text-lg hover:bg-[#cf4d1d] transition-all shadow-xl shadow-orange-900/20 hover:-translate-y-1 touch-manipulation active:scale-95" onClick={() => setViewState(ViewState.CREATE_EVENT)}>Start Hosting</button>
         </div>
      </section>
