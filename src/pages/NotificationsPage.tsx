...
import { Bell, Calendar, MessageCircle, Users, Megaphone, BarChart2, ChevronLeft, CheckCircle2, Filter } from 'lucide-react';
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center mb-8 sm:mb-12">
          ...
          <p className="text-gray-300 text-base sm:text-lg max-w-2xl mx-auto">
            Stay updated on your events, messages, and activity
          </p>
        </div>
        ...
        <div className="flex items-center justify-between mb-6 sm:mb-8">
          ...
          <div className="flex gap-2 sm:gap-3">
            <button
              ...
              className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full font-medium text-sm transition-all touch-manipulation active:scale-95 ${
                filter === 'all'
            ...
            <button
              ...
              className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-full font-medium text-sm transition-all touch-manipulation active:scale-95 flex items-center gap-2 ${
                filter === 'unread'
                  ? 'bg-[#e35e25] text-white shadow-md'
                  : 'bg-white/5 text-gray-300 hover:bg-white/10 border border-white/10'
              }`}
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="px-4 py-2 bg-white/5 border border-white/10 rounded-full text-sm font-medium text-gray-300 hover:bg-white/10 transition-colors flex items-center gap-2 touch-manipulation active:scale-95 disabled:opacity-50"
            >
        ...
        {/* Notifications List */}
        {loading ? (
          <div className="bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 p-12 text-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-white mx-auto mb-4"></div>
            <p className="text-gray-400">Loading notifications...</p>
          </div>
        ) : filteredNotifications.length === 0 ? (
          <div className="bg-white/5 rounded-2xl sm:rounded-3xl border border-white/10 overflow-hidden">
          ...
          <div className="space-y-3 sm:space-y-4">
            {filteredNotifications.map(notification => (
              <div
                ...
                className={`rounded-2xl border transition-all cursor-pointer touch-manipulation active:scale-[0.98] ${
                  notification.read
                    ? 'bg-white/5 border-white/10 hover:bg-white/10'
                    : 'bg-[#e35e25]/10 border-[#e35e25]/30 hover:bg-[#e35e25]/15'
                }`}
              >
                <div className="p-4 sm:p-5 flex gap-4">
                  {/* Icon */}
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${
                    notification.read ? 'bg-white/10' : 'bg-[#e35e25]/20'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      {!notification.read && (
                        <div className="w-2.5 h-2.5 bg-[#e35e25] rounded-full flex-shrink-0 mt-2"></div>
                      )}
                    </div>
